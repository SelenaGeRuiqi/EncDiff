...
1. **lr选择**：所有模块共用 lr=2e-7（claude推荐的），但 MCL（Pi_g, Pi_u, MechanismCritic）是从随机初始化训起的，是否需要给它们单独设一个更高的 lr？

2. **Experiment计划是否ok**：
- run_mcl_parallel_amd.sh
- plan: 在之前训练好的concat EncDiff（VQ-VAE concat + diffusion，无 MCL）的基础上引入MCL先finetune3个epoch(先看看MCL loss 在不在降，5 种 loss type 哪个表现好，λ=0.01 和 0.05 哪个更合适，然后再后续调整跑更多的)
...
- setting：max_epochs: 3，batch_size: 128，lr: 2e-7，WandB 记录 loss_mcl, loss_simple, mcl_diffusion_ratio（监控是否有两个obj梯度collapse）, factor_vae_score, dci_disentanglement

3. **Implementation有无问题**：下面是我改动的主要内容，可以看看有没有问题。主要都集中在ddpm_enc.py
...
（2） forward  
- 在 warping 前调用 Encoder4.encoding(c) 得到 u=(B,20)，存到 self._mcl_disentangled_repr（为了维度匹配+对比学习的正负样本基于真实的 disentangled factors）
