...
from ldm.models.diffusion.mcl_utils import MLPProj, MechanismCritic, mcl_loss
...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        # self.logvar.cuda()
        if eval_name == "shapes3d":
            import data.ground_truth.shapes3d as dshapes3d
            self.label_dataset = dshapes3d.Dataset(np.arange(0,480000))
        elif eval_name == "mpi3d":
            import data.ground_truth.mpi3d as dmpi3d
            self.label_dataset = dmpi3d.Dataset(np.arange(0,1036800))
        elif eval_name == "cars3d":
            import data.ground_truth.cars3d as dcars3d
            self.label_dataset = dcars3d.Dataset(np.arange(0,17568))
    ...
    def register_schedule(self, given_betas=None, beta_schedule="linear", timesteps=1000,
        ...
        alphas_cumprod_next = np.append(alphas_cumprod[1:], alphas_cumprod[-1])
    ...
    def validation_step(self, batch, batch_idx):
        x, oc = self.get_input(batch, self.first_stage_key)
        if self.model.conditioning_key is not None:
            assert oc is not None
            if self.cond_stage_trainable:
                c = self.get_learned_conditioning(oc)
                orc = self.cond_stage_model.encoding(oc)
            if self.shorten_cond_schedule:  # TODO: drop this option
                tc = self.cond_ids[t].to(self.device)
                c = self.q_sample(x_start=c, t=tc, noise=torch.randn_like(c.float()))
        c = c.reshape(c.shape[0], self.model.diffusion_model.latent_unit, -1)
        self.validation_step_outputs.append(c.detach().cpu().numpy())
        self.validation_step_scalars.append(orc.detach().cpu().numpy())
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        if self.use_mcl and self.lambda_mcl > 0:
            ...
            # obtain u_dim from cond_stage_model or config
            ...
            u_dim = latent_unit  # MCL uses raw (B, 20) disentangled repr as u

            # Pi_g and Pi_u (same as senior's test: Pi_u takes u_dim, not warped dim)
            self.Pi_g = MLPProj(in_dim=pi_g_in_dim, out_dim=mcl_proj_dim, layernorm=True)
            self.Pi_u = MLPProj(in_dim=u_dim, out_dim=mcl_proj_dim, layernorm=False)

            # MechanismCritic: learned critic(x_hat, z, u) as in senior's code
            ...
            print(f"[MCL] neg_mode={mcl_neg_mode}")
            print(f"[MCL] Pi_g: {pi_g_in_dim} -> {mcl_proj_dim}")
            print(f"[MCL] Pi_u: {u_dim} -> {mcl_proj_dim}")
            print(f"[MCL] MechanismCritic: z_shape=({z_channels},{latent_size},{latent_size}), u_dim={u_dim}")
    ...
    def on_train_batch_start(self, batch, batch_idx, dataloader_idx=0, **kwargs):
        ...
        if self.scale_by_std and self.current_epoch == 0 and self.global_step == 0 and batch_idx == 0 and not self.restarted_from_ckpt:
            if self.first_stage_model != None and not isinstance(self.first_stage_model, IdentityFirstStage):
                ...
                del self.scale_factor
                self.register_buffer('scale_factor', torch.Tensor([1]).cuda())
                print(f"setting self.scale_factor to {self.scale_factor}")
                print("### USING STD-RESCALING ###")
        ...
        if self.model.conditioning_key is not None:
            ...
            if not self.cond_stage_trainable or force_c_encode:
                if isinstance(xc, dict) or isinstance(xc, list):
                    ...
                    if return_false:
                        cb = self.cond_stage_model.encoding(xc.to(self.device))
            else:
                c = xc
            if bs is not None:
                c = c[:bs]
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False, return_context=False):
        ...
        if hasattr(self, "split_input_params"):
            ...
            x_recon = self.model(x_noisy, t, **cond, return_context=return_context)
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        if getattr(self, 'use_mcl', False) and getattr(self, 'lambda_mcl', 0) > 0:
            # u = exact disentangled repr (B, 20), computed in forward() before warping.
            # All MCL components (decoder_G, critic, Pi_u) use the same u,
            # following the senior's unified design.
            u_mcl = self._mcl_disentangled_repr  # (B, 20)

            def decoder_G(z, u_cond):
                return self.differentiable_decode_first_stage(
                    z, disentangled_repr=u_cond
                )
    ...
    @torch.no_grad()
    def sample_log_attn(self,cond,batch_size,ddim, ddim_steps,return_context=False,**kwargs):

        if ddim:
            ddim_sampler = DDIMSamplerAttn(self)
            shape = (self.channels, self.image_size, self.image_size)
            samples, intermediates =ddim_sampler.sample(ddim_steps,batch_size,
                                                        shape,cond,verbose=False,return_context=return_context,**kwargs)

        else:
            samples, intermediates = self.sample(cond=cond, batch_size=batch_size,
                                                 return_intermediates=True,return_context=return_context,**kwargs)
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
                   quantize_denoised=True, inpaint=True, plot_denoise_rows=False, plot_progressive_rows=True, sample_swap = False,
                   plot_diffusion_rows=True, **kwargs):

        use_ddim = ddim_steps is not None

        log = dict()
        z, c, x, xrec, xc, orc = self.get_input(batch, self.first_stage_key,
        ...
        if sample_swap:
            ...
            with self.ema_scope("Plotting Swapping"):
                for cdx in range(self.model.diffusion_model.latent_unit):
                    swapped_c = orc.clone()
                    if len(swapped_c.shape) == 2:
                        swapped_c = torch.stack(swapped_c.chunk(self.model.diffusion_model.latent_unit, dim=1), dim=1)
                    swapped_c[:,cdx] = swapped_c[0,cdx][None,:].repeat(orc.shape[0],1)
                    swapped_c = self.cond_stage_model.warp(swapped_c)
                    samples, z_denoise_row = self.sample_log(cond=swapped_c.reshape(orc.shape[0],-1),batch_size=N,ddim=use_ddim,
        ...
        if sample:
            # get denoise row
            with self.ema_scope("Plotting"):
                # print([c.shape,N, use_ddim, ddim_steps, ddim_eta])
    ...
    def configure_optimizers(self):
        ...
        #     {
        #         'params': [parameters[i] for i in range(len(parameters)) if i != 4],
        #         'lr': lr
        #     },
        #     {
        #         'params': [parameters[4]],
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None, return_context=False):
        if self.conditioning_key is None:
            out = self.diffusion_model(x, t, context=c_crossattn[0], return_context=return_context)
        elif self.conditioning_key == 'concat':
            xc = torch.cat([x] + c_concat, dim=1, return_context=return_context)
            out = self.diffusion_model(xc, t)
        elif self.conditioning_key == 'crossattn':
            # cc = torch.cat(c_crossattn, 1)
            out = self.diffusion_model(x, t, context=c_crossattn, return_context=return_context)
        elif self.conditioning_key == 'hybrid':
            xc = torch.cat([x] + c_concat, dim=1)
            cc = torch.cat(c_crossattn, 1)
            out = self.diffusion_model(xc, t, context=cc, return_context=return_context)
        elif self.conditioning_key == 'adm':
            cc = c_crossattn[0]
            out = self.diffusion_model(x, t, y=cc, return_context=return_context)
        else:
            raise NotImplementedError()
