...
def eval_func(eval_dataset, data_tensor, metric_folder, it, preflix=""):
    ...
    if len(data_tensor.shape) == 3:
        for i in range(data_tensor.shape[1]):
            ...
            pca_result = pca.fit_transform(data_tensor[:,i,:])
...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="the number of tokens used in model",
    )
    return parser
...
class ImageLogger(Callback):
    def __init__(self, batch_frequency, max_images, clamp=True, increase_log_steps=True,
                 ...
                 log_images_kwargs=None, log_config=None):
        super().__init__()
        ...
        if log_config:
            self.cus_logger = instantiate_from_config(log_config)
        else:
            self.cus_logger = None
...
if __name__ == "__main__":
    ...
    if opt.resume:
        ...
        if opt.name:
            ...
            name = ""
        if hasattr(config.model.params, "beta_schedule"):
            if config.model.params.beta_schedule == "cosine":
                now = now + "_" + "Cos" + "_"
            elif config.model.params.beta_schedule == "sqrt":
                now = now + "_" + "Sqrt" + "_"
            elif config.model.params.beta_schedule == "sqrt_linear":
                now = now + "_" + "SqLn" + "_"
        if opt.token_num == -1:
            nowname = now + name + opt.postfix + str(opt.seed)
        else:
            nowname = now + "_TK" + str(opt.token_num) + "_" + name + opt.postfix + str(opt.seed)
    ...
    # model
    if opt.token_num != -1:
        config.model.params.unet_config.params.latent_unit = opt.token_num
        token_dim = config.model.params.unet_config.params.context_dim
        if config.model.params.cond_stage_config != '__is_unconditional__':
            config.model.params.cond_stage_config.params.latent_dim = opt.token_num*token_dim
    ...
    default_callbacks_cfg = {
        ...
        "learning_rate_logger": {
            "target": "main_val.LearningRateMonitor",
            "params": {
