...
import torch
from lfw_src.metric import maximum, MultiTaskMetric
...
def eval_multitask(eval_loader, encoder, classifier, mean=None, std=None, attr_idx=-1, avg=False, mask=None):
    y_gt = []
    y_pred = []
    encoder.eval()
    classifier.eval()
...
def eval_regression(eval_loader, encoder, classifier):
    y_gt = []
    y_pred = []
    imgs_list = []
    encoder.eval()
